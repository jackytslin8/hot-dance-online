...
const SONGS = [
    ...
    { id: 4, title: '🔥 Neon City Rave', bpm: 128, type: 'mp3', url: 'assets/neon-city-rave.mp3' },
    { id: 5, title: '💎 Electronic Future', bpm: 126, type: 'mp3', url: 'assets/electronic-future.mp3' },
    { id: 6, title: '🌙 Night Drive', bpm: 110, type: 'mp3', url: 'assets/night-drive.mp3' },
    { id: 7, title: '🎧 Upbeat Energy', bpm: 120, type: 'mp3', url: 'assets/upbeat-energy.mp3' },
    { id: 8, title: '🌟 Synthwave Dream', bpm: 110, type: 'mp3', url: 'assets/synthwave-dream.mp3' },
...
class AudioEngine {
    ...
    _connectAnalyser() {
        if (!this.bgmAudio || !this.ctx) return;
        try {
            // 強制確保 source 只有一次建立
            ...
            console.log('Analyser linked successfully');
        } catch(e) {
            // 如果 createMediaElementSource 報錯-說明已連接-嘗試直接路由（如果可能）
            console.warn('Analyser connection error:', e.message);
            this.analyser = null;
            // 由於 MediaElementSource 一旦建立就不可更改
            // 如果在這裡報錯且之前沒成功，通常是因為 browser限制
            // 我們不再嘗試重複建立，避免拋出 "already connected" 錯誤
        }
    }
    // 偵測重拍（低頻能量突增）並追蹤 BPM
    detectBeat() {
